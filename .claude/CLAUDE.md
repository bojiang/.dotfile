# 最高SOP
你的每次任务都必须遵循以下SOP：
1. 一切代码修改前，先输出用户指令的类型
2.1 提问类型 -> 
    1. 先批判性思考，输出问题拆分
    2. 输出基于证据的事实核查
    3. 最后给出简短答案。答案只能是确定的结论或“信息不够”，任何时候都不允许出现猜测
2.2 代码修改类型（不含 .agent 知识写入，知识写入用户说写就直接写） -> 
    2.2.1 分析用户的提示，输出用户的提示背后的意图
    2.2.2 分析需要的信息，输出核查清单
    2.2.3 通过读取代码等操作进行事实核查，输出现状
    2.2.4 等待用户确认你对现状的理解
    2.2.5 输出修改方案简报，报告不应包含代码
    2.2.6 等待用户确认修改
    2.2.7 进行修改，不做任何用户不要求的修改
2.3 benchmark/评估任务 → 使用 benchmark skill，禁止手操步骤
3. 不要试图手动测试、构建。我的仓库一般都有Makefile或npm command 设置好了构建指令
4. 设计阶段不需要考虑向前兼容
5. 解决问题时，先思考减法能否解决问题。很多问题的根源是状态不正交。减法的修复思路可以保证之后的可维护性
6. git 多 agent 并行：任何 git 读操作先确认当前分支是预期分支；任何 git 写/提交先 fetch 并确认分支干净
7. worktree 隔离：如果当前目录有未提交代码（.agent/*、文档不算），必须在独立 git worktree 中进行，路径统一放 `.worktrees/<name>`；每波次修改后提交commit合入main，不在 worktree 停留
8. 直接输出样子 > 用语言间接描述

# 目录
你所处的目录遵守以下约定：
AGENT.md     # 最高SOP，每次进入项目时都读取, 里面不包含细节，优先索引
.agent
  state/
    INDEX.md             # 项目状态总索引，SessionStart hook 自动加载。规则：
                         #   1. 尽量短，以引用为主；只有高频使用的信息才内联
                         #   2. 只记录特殊信息，默认/显而易见的不记录
                         #   3. state/ 下每个文件都必须从 INDEX.md 直接或间接（经子索引）可达
    <topic>/             # 按主题组织，每主题若干 aspect 子目录，各自 INDEX.md
      requirement/       # 你对用户需求的理解，来源：用户提示和你的分析. 你会永远记得更新它
        INDEX.md
        ...              # 其他需求细节文档
      design/            # 设计决策与形态（pen 设计稿也放这里，前端实现时的最高参考）
        INDEX.md
        ...
      usage/             # 如何使用/操作该主题的产出（如何运行、如何扩展）
        INDEX.md
        ...
  backlogs/              # 任务记录. 你不会把它当做当前的ground truth
    001_<name>/
      INDEX.md           # 任务细节的索引
      ...                # 其他任务细节文档
  benchmarks/            # bench 类任务。详见 benchmark skill
<git repo1>/
<git repo2>/
...
你可以注意到，比起你熟悉的环境，这个目录通常在repo的上一级，这多个repo之间是有业务上的联系的
`.agent` 的查找规则只有一条：`$CWD/.agent`。"repo 上一级"只是典型启动位置，不是查找规则；任何组件（hook、subagent）都不向上级目录查找
项目相关记忆都避免使用 Claude 的 auto-memory，应该使用上述结构
任务收尾时你判断：本轮是否产生了"花了成本得到、且之后稳定成立"的结论？有 → 派 state-keeper subagent 写入，交接格式固定为（结论 / why / 涉及文件）；无 → 直接结束
你不直接读写 state/ 细节文件——读旧状态、去重合并、维护索引都在 state-keeper 的子上下文里发生，保持主上下文短

# 情绪感知
当用户表现出惊讶或意外（"不是吧"、"你不知道吗"、"之前不是说过"、"怎么会"等），或者用户所提供的信息理论上你应该从 .agent 目录里已经知道时，说明你可能缺少关键上下文。此时：
1. 主动读取 .agent/state/INDEX.md 及相关子索引
2. 评估是否需要更新 state 记录（implementation/requirements）以避免未来再次遗漏

# 代码风格
默认代码风格：
1. 一切外部调用（API、命令等）需要在调试日志包括原始外部输入和输出、原始错误
2. 默认不做任何向前兼容设计
3. 极少注释，只注释“why”，不注释“how”等
4. 默认只使用英文注释和英文输出

# 指南索引
- archemind.org 文章发布: /Users/agent/workspace/mana/.agent/skills/archemind-publish.md
